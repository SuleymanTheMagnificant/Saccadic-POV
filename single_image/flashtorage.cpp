...
void msc_init(){
  ...
  uint32_t block_count = fatfs.card()->sectorCount();
#else
  uint32_t block_count = fatfs.card()->cardSize();
#endif
...
int32_t msc_read_cb (uint32_t lba, void* buffer, uint32_t bufsize){
    ...
    #if SD_FAT_VERSION >= 20000
      rc = fatfs.card()->readSectors(lba, (uint8_t*) buffer, bufsize/512);
    #else
      rc = fatfs.card()->readBlocks(lba, (uint8_t*) buffer, bufsize/512);
    #endif
...
int32_t msc_write_cb (uint32_t lba, uint8_t* buffer, uint32_t bufsize){
  ...
  #if SD_FAT_VERSION >= 20000
    rc = fatfs.card()->writeSectors(lba, buffer, bufsize/512);
  #else
    rc = fatfs.card()->writeBlocks(lba, buffer, bufsize/512);
  #endif
...
void msc_flush_cb (void){
    #if SD_FAT_VERSION >= 20000
    fatfs.card()->syncDevice();
  #else
    fatfs.card()->syncBlocks();
  #endif
